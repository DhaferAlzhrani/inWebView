    ...
    uint8_t *tag)
{
    ...
    r = encrypt_full(
            AES_GCM_256,                   // same mode used in test_reference
            (uint8_t *)key,
            (uint8_t *)iv,   iv_len  * 8,   // bit length!
            (uint8_t *)aad, aad_len * 8,
            (uint8_t *)plain, plain_len * 8,
            (uint8_t *)crypt,
            (uint8_t *)tag
        );

    return (r == SUCCESSFUL_OPERATION) ? 0 : -1;
}
